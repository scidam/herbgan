...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
                nn.ConvTranspose2d(1, 512, DECONV_KERNEL_SIZE, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(512),
                nn.ReLU(True),
                nn.ConvTranspose2d(512, 256, DECONV_KERNEL_SIZE, stride=1, padding=1, bias=False),
                nn.BatchNorm2d(256),
                nn.ReLU(True),
                nn.ConvTranspose2d(256, 3, DECONV_KERNEL_SIZE, stride=1, padding=1, bias=False),
                nn.Tanh()
                )
        
    def forward(self, z):
        z = z.view(-1, 1, IMAGE_SIZE, IMAGE_SIZE)
        img = self.main(z)
        return img
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
                ...
                nn.BatchNorm2d(128),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(128, 144, 8, stride=2, padding=1, bias=False),
                nn.BatchNorm2d(144),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(144, 196, 8, stride=2, padding=1, bias=False),
                nn.BatchNorm2d(196),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(196, 256, 6, stride=2, padding=1, bias=False),
                nn.BatchNorm2d(256),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
...
for epoch in range(NUM_EPOCHES):
    for ii, (real_images, train_labels) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        # train with fake
        noise = torch.randn(batch_size, 1, IMAGE_SIZE, IMAGE_SIZE, device=device)
        ...
        if (ii+1) % (len(train_loader)//2) == 0:
            print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f D(x): %.4f D(G(z)): %.4f / %.4f'
                  % (epoch + 1, NUM_EPOCHES, ii+1, len(train_loader),
...
noise = torch.randn(batch_size, 1, IMAGE_SIZE, IMAGE_SIZE, device=device)
fake = Generator_net(noise)
img = fake.detach().cpu().numpy().transpose(0, 2, 3, 1)
plt.imshow(img[0])
plt.show()